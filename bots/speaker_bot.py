...
class SpeakerBot:
    ...
    def _setup_events(self):
        ...
        async def on_voice_state_update(member, before, after):
            ...
            if before.channel and before.channel.id == self.channel_id:
                logger.info(
                    f"Voice state update in speaker channel: {member.display_name}"
                )
                logger.info(
                    f"Before: muted={before.self_mute}, deafened={before.self_deaf}"
                )
                logger.info(
                    f"After: muted={after.self_mute}, deafened={after.self_deaf}"
                )
                ...
                if member.id == self.bot.user.id:
                    if after.channel is None:
                        ...
                        logger.warning(
                            f"Bot was moved to different channel: {after.channel.name}"
                        )
    ...
    async def _start_websocket_server(self):
        ...
        try:
            ...
            logger.info(
                f"Started WebSocket server on port {port} for speaker bot {self.bot_id}"
            )
