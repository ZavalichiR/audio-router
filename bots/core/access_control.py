...
class AccessControl:
    ...
    def _load_access_config(self):
        ...
        if roles_env:
            self.authorized_roles = set(role.strip() for role in roles_env.split(",") if role.strip())
        
        # Load authorized user IDs
        users_env = os.getenv("AUTHORIZED_USERS", "")
        if users_env:
            self.authorized_users = set(int(user_id.strip()) for user_id in users_env.split(",") if user_id.strip())
        
        logger.info(f"Loaded access control: {len(self.authorized_roles)} roles, {len(self.authorized_users)} users")
    ...
    async def create_authorized_role(self, guild: discord.Guild, role_name: str = "Broadcast Controller") -> Optional[discord.Role]:
        """
    ...
    async def setup_private_channel_permissions(self, channel: discord.TextChannel, 
                                              authorized_role: Optional[discord.Role] = None) -> bool:
        """
        ...
        try:
            ...
            if not bot_member.guild_permissions.manage_channels:
                logger.warning(f"Bot lacks 'Manage Channels' permission in {channel.guild.name}")
                return False
            ...
            try:
                ...
                logger.warning(f"Cannot modify @everyone permissions for {channel.name}")
                return False
            ...
            if authorized_role:
                try:
                    ...
                    if bot_member.top_role > authorized_role:
                        ...
                        logger.warning(f"Cannot modify role {authorized_role.name} - bot role too low")
                except discord.Forbidden:
                    logger.warning(f"Cannot set permissions for role: {authorized_role.name}")
            ...
            for member in authorized_members:
                try:
                    ...
                    if bot_member.top_role > member.top_role:
                        ...
                        logger.info(f"Set permissions for member: {member.display_name}")
                    else:
                        logger.warning(f"Cannot modify permissions for {member.display_name} - bot role too low")
                except discord.Forbidden:
                    logger.warning(f"Cannot set permissions for member: {member.display_name}")
