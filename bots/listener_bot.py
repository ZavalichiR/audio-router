...
logger = setup_logging(
    component_name="listener_bot",
    log_level="INFO",
    log_file="logs/listener_bot.log"
)
...
class ListenerBot:
    ...
    async def _connect_to_speaker(self):
        ...
        try:
            ...
            logger.info(f"Connecting to speaker WebSocket: {self.speaker_websocket_url}")
    ...
    async def _listen_for_audio(self):
        ...
        while True:
            try:
                async for message in self.websocket:
                    try:
                        ...
                        if data.get("type") == "welcome":
                            logger.info(f"Received welcome from speaker: {data.get('speaker_id')}")
                            ...
                            if audio_hex and self.audio_buffer:
                                ...
                                logger.debug(f"🎧 Received and buffered audio: {len(audio_data)} bytes")
                            else:
                                logger.warning("Received audio message but no audio data or buffer")
                ...
                logger.warning("Speaker WebSocket connection closed, attempting to reconnect...")
                break
            except Exception as e:
            ...
            try:
                ...
                logger.error(f"Failed to reconnect to speaker WebSocket: {reconnect_error}")
                logger.error(f"Reconnection error type: {type(reconnect_error).__name__}")
                self._reconnecting = False
    ...
    async def connect_to_channel(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Connected to listener channel: {channel.name} (self-deafened)")
            return True
