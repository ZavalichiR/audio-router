...
class SimpleConfigManager:
    ...
    def _get_listener_tokens(self) -> List[str]:
        ...
        if not tokens:
            logger.warning("No listener bot tokens found, will use main token for all bots")
    ...
    def _get_authorized_users(self) -> List[int]:
        ...
        if users_env:
            try:
                return [int(user_id.strip()) for user_id in users_env.split(",") if user_id.strip()]
            except ValueError:
    ...
    def get_config(self) -> SimpleConfig:
        ...
        try:
            ...
            config = SimpleConfig(
                ...
                create_authorized_role=self._get_optional_env("CREATE_AUTHORIZED_ROLE", "true").lower() == "true",
                authorized_role_name=self._get_optional_env("AUTHORIZED_ROLE_NAME", "Broadcast Controller")
            )
