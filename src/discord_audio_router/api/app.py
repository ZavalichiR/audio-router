...
def get_subscription_manager() -> SubscriptionManager:
    ...
    if subscription_manager is None:
        raise HTTPException(
            status_code=500, detail="Subscription manager not initialized"
        )
    return subscription_manager


def create_app(
    bot_token: Optional[str] = None, db_path: str = "data/subscriptions.db"
) -> FastAPI:
    """
    ...
    async def create_subscription(
        ...
        try:
            ...
            # Create subscription
            success = await manager.create_subscription_from_invite(
                request.invite_code, tier
            )
            if not success:
                raise HTTPException(
                    status_code=500,
            ...
            server_id = await manager.discord_api.get_server_id_from_invite(
                request.invite_code
            )
            if not server_id:
                raise HTTPException(
                    status_code=500, detail="Failed to resolve server ID"
                )

            subscription = manager.get_server_subscription(server_id)
            if not subscription:
                raise HTTPException(
                    status_code=500, detail="Subscription not found after creation"
                )
    ...
    async def get_subscription(
        server_id: str, manager: SubscriptionManager = Depends(get_subscription_manager)
    ):
        """
    ...
    async def update_subscription(
        ...
        try:
            ...
            if not subscription:
                raise HTTPException(
                    status_code=500, detail="Subscription not found after update"
                )
    ...
    async def validate_listener_count(
        ...
        return ListenerValidationResponse(
            is_valid=is_valid, max_allowed=max_allowed, message=message
        )
