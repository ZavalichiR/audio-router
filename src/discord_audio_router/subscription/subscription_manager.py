...
class SubscriptionManager:
    ...
    def __init__(self, db_path: str = "data/subscriptions.db", bot_token: Optional[str] = None):
        """
    ...
    def get_server_max_listeners(self, server_id: str) -> int:
        ...
        if subscription:
            ...
            if subscription.tier == SubscriptionTier.CUSTOM and subscription.max_listeners == 0:
    ...
    def get_subscription_by_invite(self, invite_code: str) -> Optional[ServerSubscription]:
        """
    ...
    async def create_subscription_from_invite(self, invite_code: str, tier: SubscriptionTier) -> bool:
        """
        ...
        try:
            ...
            if not server_id:
                logger.error(f"Could not resolve invite code {invite_code} to server ID")
                return False
            ...
            logger.error(f"Failed to create subscription from invite {invite_code}: {e}")
            return False
    
    def update_server_subscription(self, server_id: str, tier: SubscriptionTier) -> bool:
        """
    ...
    def update_subscription_by_invite(self, invite_code: str, tier: SubscriptionTier) -> bool:
        """
        ...
        try:
            ...
            logger.error(f"Failed to update subscription for invite code {invite_code}: {e}")
            return False
    ...
    def delete_subscription_by_invite(self, invite_code: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to delete subscription for invite code {invite_code}: {e}")
            return False
    ...
    def validate_listener_count(self, server_id: str, requested_count: int) -> tuple[bool, int, str]:
        """
