...
class SubscriptionDatabase:
    ...
    def get_subscription_by_server_id(self, server_id: str) -> Optional[ServerSubscription]:
        """
    ...
    def get_subscription_by_invite_code(self, invite_code: str) -> Optional[ServerSubscription]:
        """
        ...
        try:
            ...
            logger.error(f"Failed to get subscription by invite code {invite_code}: {e}")
            return None
    ...
    def create_subscription(self, subscription: ServerSubscription) -> bool:
        ...
        try:
            ...
            logger.warning(f"Subscription already exists for server {subscription.server_id}: {e}")
            return False
        except Exception as e:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("""
                    ...
                    WHERE server_id = ?
                """, (subscription.tier.value, subscription.max_listeners, subscription.server_id))
                
                if cursor.rowcount > 0:
                    conn.commit()
                    logger.info(f"Updated subscription for server {subscription.server_id}")
                    return True
                else:
                    logger.warning(f"No subscription found for server {subscription.server_id}")
                    return False
    ...
    def delete_subscription(self, server_id: str) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute("DELETE FROM subscriptions WHERE server_id = ?", (server_id,))
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in cursor.fetchall():
                    subscriptions.append(ServerSubscription(
                        invite_code=row[0],
                        server_id=row[1],
                        tier=SubscriptionTier(row[2]),
                        max_listeners=row[3],
