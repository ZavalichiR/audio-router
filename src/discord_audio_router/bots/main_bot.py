...
# Global audio router and subscription manager instances
...
async def get_available_receiver_bots_count(guild: discord.Guild) -> int:
    ...
    try:
        ...
        receiver_bot_count = sum(
            1 for member in members if member.bot and member.display_name.startswith("Rcv-")
        )
...
async def setup_roles_command(ctx):
    ...
    try:
        ...
        if broadcast_admin_role:
            member_count = sum(1 for m in ctx.guild.members if broadcast_admin_role in m.roles)
            result_embed.add_field(
...
async def check_setup_command(ctx):
    ...
    try:
        ...
        role_status = []
        if speaker_role:
            role_status.append(f"✅ Speaker Role ({sum(1 for m in ctx.guild.members if speaker_role in m.roles)} members)")
        else:
            role_status.append("❌ Speaker Role (Missing)")
        if broadcast_admin_role:
            role_status.append(f"✅ Broadcast Admin Role ({sum(1 for m in ctx.guild.members if broadcast_admin_role in m.roles)} members)")
        else:
        ...
        issues = []
        if not has_administrator and (not perms.manage_channels or not perms.manage_roles):
...
async def check_permissions_command(ctx):
    ...
    try:
        ...
        permissions = [
            ("Manage Channels", perms.manage_channels, "Required to create and manage voice channels"),
            ("Manage Roles", perms.manage_roles, "Required to create and manage roles"),
            ("Connect", perms.connect, "Required to join voice channels"),
            ("Speak", perms.speak, "Required to play audio in voice channels"),
            ("Send Messages", perms.send_messages, "Required to send command responses"),
            ("Read Message History", perms.read_message_history, "Required to read command messages"),
            ("Embed Links", perms.embed_links, "Required to send rich embeds"),
            ("Administrator", perms.administrator, "Full access to all features"),
        ]
...
async def role_info_command(ctx):
    ...
    try:
        ...
        broadcast_admin_role = discord.utils.get(
            ctx.guild.roles, name=audio_router.access_control.broadcast_admin_role_name
        )
