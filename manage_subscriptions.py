...
async def create_subscription(manager: SubscriptionManager, invite_code: str, tier: str):
    ...
    try:
        ...
        print(f"❌ Invalid tier '{tier}'. Valid tiers: {[t.value for t in SubscriptionTier]}")
...
def show_tiers():
    ...
    for tier, info in SUBSCRIPTION_TIERS.items():
        listeners_text = "unlimited" if info['max_listeners'] == 0 else f"{info['max_listeners']} listeners"
...
async def main():
    """Main CLI function."""
    parser = argparse.ArgumentParser(description="Manage Discord Audio Router subscriptions")
    parser.add_argument("--db-path", default="data/subscriptions.db", help="Path to subscription database")
    ...
    create_parser.add_argument("tier", help="Subscription tier (free, basic, standard, advanced, premium, custom)")
    ...
    update_parser.add_argument("tier", help="New subscription tier (free, basic, standard, advanced, premium, custom)")
