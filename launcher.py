...
class ProcessManager:
    ...
    def stop_component(self, component: str, timeout: int = 10) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(
                    f"Component {component} did not stop gracefully, forcing shutdown"
                )
                process.kill()
                process.wait()
    ...
    def start_main_bot(self) -> bool:
        ...
        return self.start_component(
            "audiobroadcast_bot", ["-m", "discord_audio_router.bots.main_bot"]
        )

    def start_relay_server(self) -> bool:
        """Start the WebSocket relay server."""
        return self.start_component(
            "relay_server", ["-m", "discord_audio_router.networking.websocket_server"]
        )
...
def main():
    ...
    parser.add_argument(
        "--config", "-c", default=".env", help="Configuration file path"
    )
    parser.add_argument(
        "--component",
        help="Start specific component (audiobroadcast_bot, relay_server)",
    )
    parser.add_argument(
        "--no-relay", action="store_true", help="Don't start relay server"
    )
    parser.add_argument(
        "--monitor", action="store_true", help="Enable health monitoring"
    )
